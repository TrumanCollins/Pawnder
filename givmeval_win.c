...
extern int do_second_level_sort;
...
//        int            deeper_flag_set_at_this_level = False;
...
//                if(value != capture_extension_value) {
                    delete_position(posit);
                    return(value);
//                } else {
//                    depth++;
...
//            }
        ...
        if(Use_Hashtable && curr_depth > 2 && glob__hash_table->check_table(&posit->hash_board, &value, curr_depth, alpha_beta, comp_color)) {
            delete_position(posit);
            if(Debug7) *best_line = NULL;
//            if(deeper_flag_set_at_this_level) glob__dont_go_any_deeper = False;
            return(value);
        }
#endif
...
//if(move->next_lev_moves) cerr << "Unused next level move.\n";
            if(curr_depth == depth - 1) {
//            if(curr_depth == depth) {
//            if(curr_depth >= glob__max_main_level) {
                glob__curr_level = 0;
            } else {
                glob__curr_level = curr_depth;
            }
//            if(extension == capture_extension_value) {
//                glob__gen_just_captures = True;
//                move_top = posit->gen_moves();
//                glob__gen_just_captures = False;
//
//                // Now check for the case when there is a check and no captures.
//
//                if(!move_top) {
//                    move_top = posit->gen_moves();
//                    glob__dont_go_any_deeper = True;
//                    deeper_flag_set_at_this_level = True;
//                }
//            } else {
                move_top = posit->gen_moves();
//            }
...
glob__alpha_betas[curr_depth + 2] = glob__low_window;
                value   = -max_value;
...
//                    if(value > alpha_beta + 50 && curr_depth > 2 && curr_depth < depth) {
//                        glob__killers.save_killer(curr_depth, temp);
//                    }
//                    if(value > alpha_beta + 50 && curr_depth < glob__max_main_level && curr_depth > 2) {
                        glob__killers.save_killer(curr_depth, temp);
                    }
                    temp->val = value;
                    ...
                    if(temp1->val <= temp->val) {
                        ...
                        temp1 = temp;
                    } else if(Debug7) free_list(best_moves_below);
                    last_evaluated = temp;
                    temp = temp->next;
                }
            } else {
//                glob__alpha_betas[curr_depth + 2] = max_value - 1;
glob__alpha_betas[curr_depth + 2] = glob__high_window;
                value   = max_value;
...
//            if(curr_depth < glob__max_main_level && curr_depth > 2) {
//                glob__killers.save_killer(curr_depth, temp1);
//            }
            ret_val = temp1->val;
            if(Debug7) {
